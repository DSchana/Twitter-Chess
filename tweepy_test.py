...
# Shahir Chowdhury
# 2017-01-21
# This program handles the twitter side of the TwitChess
#
# TO DO:
...
class Input:
	...
	#This function obtains new tweets and adds the corresponding actions to 
	def update(self):
		for tweet in tweepy.Cursor(api.search, q = '@realtwitchess ',count = 100,lang = ' ').items():
			try:
				text = str(tweet.text)
				textparts = str.split(text) #convert tweet into string array

				for x, string in enumerate(textparts): 
					if (x < len(textparts)-1):
						if string == "gamestart" and textparts[x+1][:1] == "@":
...
#test.sendMessage("g")
#test.sendMessage("g",822095735411896325)
#test.sendMessage("g",822095735411896325,735128411874689024)
